    ...
    for epoch_num in range(parser.epochs):
        ...
        for  it,data in enumerate(dataloader_train):
            try:
                ...
                it_mins, it_secs = it_time(start_time, end_time)

                print(
                    '\tit:{}/{} | {}m {}s | Classification loss: {:1.5f} | Regression loss: {:1.5f} | Running loss: {:1.5f}'.format(
                         int((it+1)*4),int(len(dataset_train)),int(it_mins),int(it_secs),float(classification_loss), float(regression_loss), np.mean(loss_hist)))
...
def it_time(start_time, end_time):
